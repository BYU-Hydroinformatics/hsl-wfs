...
class Wfs extends CI_Controller
{
	...
	function write_xml()
	{
		...
		case 'DescribeFeatureType':
			...
			switch($version):
				case '1.0.0' : 
				//Check for OP Format exception
				if($this->input->get('outputFormat'))
				{
					if ($this->input->get('outputFormat') != 'xml')
					{
						$data['message']=' <ServiceException code="UnsupportedFormat" locator="DescribeFeatureType">
      The output format is not supported.';
						$this->load->view('describeFT_error',$data);
						break;
					}
				}
				
				//Check for TypeName Exception
				
				$typeNames = get_TypeName();
				/*
				if ($typeNames != 'hydro_site') ##Possibly just add a list to verify against. 
					{
						$data['message']=' <ServiceException code="UnsupportedTypeName" locator="DescribeFeatureType">
      This TypeName is not supported.';
						$this->load->view('describeFT_error',$data);
						break;
					}
				*/
				$this->load->view('describe_feature_type'); break;
